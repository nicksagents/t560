...
function resolveExecConfig(params: { cfg?: T560Config; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: T560Config; agentId?: string }) {
  const cfg = params.cfg;
...
export function createT560CodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createT560ReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
