...
import {
  ...
  normalizeStringList,
  parseT560ManifestInstallBase,
  parseFrontmatterBool,
  resolveT560ManifestBlock,
  resolveT560ManifestInstall,
  resolveT560ManifestOs,
  resolveT560ManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseT560ManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveT560Metadata(
  frontmatter: ParsedSkillFrontmatter,
): T560SkillMetadata | undefined {
  const metadataObj = resolveT560ManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveT560ManifestRequires(metadataObj);
  const install = resolveT560ManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveT560ManifestOs(metadataObj);
  return {
