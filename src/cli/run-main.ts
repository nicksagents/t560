...
function hasWizardCompleted(cfg: OpenClawConfig): boolean {
  const lastRunAt = cfg.wizard?.lastRunAt;
  return typeof lastRunAt === "string" && lastRunAt.trim().length > 0;
}

function normalizeAllowList(entries: unknown): string[] {
...
async function checkTelegramReadiness(cfg: OpenClawConfig): Promise<StartupReadiness> {
  ...
  if (!tokenResolution.token) {
    reasons.push(
      "Telegram bot token is missing. Set channels.telegram.botToken/tokenFile or TELEGRAM_BOT_TOKEN.",
    );
    return { ready: false, reasons };
  }
  ...
  try {
    ...
    const probe = await probeTelegram(
      ...
      3500,
      typeof telegramCfg?.proxy === "string" ? telegramCfg.proxy : undefined,
    );
    if (!probe.ok) {
    ...
    reasons.push(
      `Telegram connectivity probe failed (${err instanceof Error ? err.message : String(err)}).`,
    );
  }
...
function checkWebchatReadiness(cfg: OpenClawConfig): StartupReadiness {
  const reasons: string[] = [];
...
async function checkNoArgStartupReadiness(cfg: OpenClawConfig): Promise<StartupReadiness> {
  const reasons: string[] = [];
...
async function probeLocalGatewayPort(port: number, timeoutMs = 800): Promise<boolean> {
...
async function ensureLocalGatewayReady(cfg: OpenClawConfig): Promise<boolean> {
  ...
  console.error(
    `[t560] gateway not detected on 127.0.0.1:${port}; starting local gateway${bindMode !== "loopback" ? " (loopback override)" : ""}…`,
  );
  const child = spawn(
    process.execPath,
    startArgs,
  ...
  console.error(
    "[t560] gateway did not become ready in time; opening gateway command output. If this repeats, run: t560 gateway --force --verbose",
  );
  return false;
}

async function applyDefaultCommandForNoArgs(argv: string[]): Promise<string[]> {
