...
import { describeUnknownError, extractGatewayMiskeys, parsePort, toOptionString, } from "./shared.js";
const gatewayLog = createSubsystemLogger("gateway");
async function runGatewayCommand(opts) {
    const isDevProfile = process.env.T560_PROFILE?.trim().toLowerCase() === "dev";
    ...
    if (opts.claudeCliLogs) {
        ...
        process.env.T560_CLAUDE_CLI_LOG_OUTPUT = "1";
    }
    ...
    if (opts.rawStream) {
        process.env.T560_RAW_STREAM = "1";
    }
    const rawStreamPath = toOptionString(opts.rawStreamPath);
    if (rawStreamPath) {
        process.env.T560_RAW_STREAM_PATH = rawStreamPath;
    }
    if (devMode) {
    ...
    if (opts.token) {
        ...
        if (token) {
            process.env.T560_GATEWAY_TOKEN = token;
        }
    }
    ...
    if (!opts.allowUnconfigured && mode !== "local") {
        if (!configExists) {
            defaultRuntime.error(`Missing config. Run \`${formatCliCommand("t560 setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`);
        }
        else {
    ...
    if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
        defaultRuntime.error([
            ...
            "Set gateway.auth.token (or T560_GATEWAY_TOKEN), or pass --token.",
            ...authHints,
        ]
            .filter(Boolean)
            .join("\n"));
    ...
    if (resolvedAuthMode === "password" && !hasPassword) {
        defaultRuntime.error([
            ...
            "Set gateway.auth.password (or T560_GATEWAY_PASSWORD), or pass --password.",
            ...authHints,
        ]
            .filter(Boolean)
            .join("\n"));
    ...
    if (bind !== "loopback" && !hasSharedSecret) {
        defaultRuntime.error([
            ...
            "Set gateway.auth.token/password (or T560_GATEWAY_TOKEN/T560_GATEWAY_PASSWORD) or pass --token/--password.",
            ...authHints,
        ]
            .filter(Boolean)
            .join("\n"));
    ...
    try {
        ...
        if (err instanceof GatewayLockError ||
            ...
            defaultRuntime.error(`Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("t560 gateway stop")}`);
            try {
