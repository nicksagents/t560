...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.T560_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.T560_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.T560_NODE_OPTIONS_READY = "1";
  const child = spawn(
    process.execPath,
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    console.error(`[t560] ${parsed.error}`);
    process.exit(2);
  }
