...
import { formatChannelPrimerLine, listChatChannels, } from "../channels/registry.js";
...
export async function setupChannels(cfg, runtime, prompter, options) {
    ...
    const compactQuickstart = options?.quickstartDefaults && options?.skipConfirm && Boolean(options?.initialSelection?.[0]);
    if (compactQuickstart) {
        await prompter.note(`QuickStart will set up ${options?.initialSelection?.[0]} now. You can add more channels later with \`${formatCliCommand("t560 channels add")}\`.`, "How channels work");
    }
    else {
