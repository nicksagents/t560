...
function formatArgs(args: unknown): string {
  ...
  if (parts.length > 0) {
    return sanitizeRenderableText(parts.join(" | "));
  }
  try {
    return sanitizeRenderableText(JSON.stringify(record));
  } catch {
    return "";
  }
}
...
function extractText(result?: ToolResult): string {
  ...
  if (Array.isArray(result.content)) {
    ...
    for (const entry of result.content) {
      if (entry.type === "text" && entry.text) {
        ...
        const size = typeof entry.bytes === "number" ? ` ${Math.round(entry.bytes / 1024)}kb` : "";
  ...
  return sanitizeRenderableText([stdout, stderr].filter(Boolean).join("\n").trim());
}
...
export class ToolExecutionComponent extends Container {
  ...
  setResult(result: ToolResult | undefined, opts?: { isError?: boolean }): void {
    this.result = result;
