...
import { detectBrowserOpenSupport, formatControlUiSshHint, openUrl, probeGatewayReachable, waitForGatewayReachable, resolveControlUiLinks, } from "../commands/onboard-helpers.js";
...
export async function finalizeOnboardingWizard(options) {
    ...
    if (!opts.skipHealth) {
        ...
        try {
            ...
            await prompter.note([
                "Docs:",
                "https://docs.t560.ai/gateway/health",
                "https://docs.t560.ai/gateway/troubleshooting",
            ].join("\n"), "Health check help");
        }
    }
    ...
    const authedUrl = settings.authMode === "token" && settings.gatewayToken
    ...
    await prompter.note([
        ...
        gatewayStatusLine,
        "Docs: https://docs.t560.ai/web/control-ui",
    ]
        .filter(Boolean)
        .join("\n"), "Control UI");
    ...
    if (!opts.skipUi && gatewayProbe.ok) {
        ...
        await prompter.note([
            ...
            "Stored in: ~/.t560/t560.json (gateway.auth.token) or T560_GATEWAY_TOKEN.",
            `View token: ${formatCliCommand("t560 config get gateway.auth.token")}`,
            `Generate token: ${formatCliCommand("t560 doctor --generate-gateway-token")}`,
            "Web UI stores a copy in this browser's localStorage.",
            `Open the dashboard anytime: ${formatCliCommand("t560 dashboard --no-open")}`,
        ...
        if (hatchChoice === "tui") {
            ...
            await prompter.note([
                ...
                controlUiOpened
                    ? "Opened in your browser. Keep that tab to control T560."
                    : "Copy/paste this URL in a browser on this machine to control T560.",
                controlUiOpenHint,
            ]
                .filter(Boolean)
                .join("\n"), "Dashboard ready");
            ...
            await prompter.note(`When you're ready: ${formatCliCommand("t560 dashboard --no-open")}`, "Later");
        }
    }
    else if (opts.skipUi) {
